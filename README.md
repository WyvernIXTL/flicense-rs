...
> [!WARNING]
> This is an experimental feature. You must be sure, that the versions of `license-fetcher` and `flicense` are compatible. 
> `license-fetcher=0.8.4` is compatible with `flicense=0.4.0`, `license-fetcher=0.9.1` is compatible with `flicense=0.5.0` and `license-fetcher=0.10.0` is compatible with `flicense=0.6.0`
